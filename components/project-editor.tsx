...
import { studioCharacters } from "@/lib/create-studio";
...
export function ProjectEditor({
  ...
  const [gameplayVideoSrc, setGameplayVideoSrc] = useState(setting(project.settings, "gameplayVideoSrc", ""));
  const [gameplayStartOffset, setGameplayStartOffset] = useState(setting(project.settings, "gameplayStartOffset", "0"));
  ...
  }), [aspectRatio, backgroundId, captionStyle, gameplayStartOffset, gameplayVideoSrc, project.settings, renderMode, speakerVoices, template, voiceIntensity, voicePreset, voiceSpeed]);
  ...
  }, [title, hook, narration, scenes, dialogue, speakerVoices, voicePreset, voiceSpeed, voiceIntensity, template, captionStyle, aspectRatio, renderMode, backgroundId, gameplayVideoSrc, gameplayStartOffset, scriptPreparing]);
    ...
    <div className="editor-page">
      ...
      <main id="main-content" className="editor-grid">
        ...
        <section className="editor-preview" aria-label={isVoice ? "Audio preview" : "Video preview"}>
          ...
          <div className="editor-canvas">
            <div className={`editor-video${isVoice ? " voice-output" : ""}`}>
              ...
              {!isVoice && renderMode === "gameplay" && !(project.outputIsFinal ? project.outputUrl : project.motionUrl) ? <GameplayPreview backgroundId={backgroundId} caption={hook} videoSrc={gameplayVideoSrc || null} startOffsetSeconds={Math.max(0, Number(gameplayStartOffset) || 0)} /> : null}
        ...
        <aside className="render-panel" aria-label="Generate and export">
          ...
          {!isVoice && generationState === "completed" && project.narrationUrl && (renderMode === "gameplay" || project.posterUrl) ? <VideoComposer projectId={project.id} title={project.title} script={project.script} posterUrl={project.posterUrl} audioUrl={project.narrationUrl} audioSegments={project.audioSegments} motionVideoUrl={renderMode === "ai-motion" ? project.motionUrl : null} durationSeconds={project.durationSeconds} captionStyle={captionStyle} backgroundId={backgroundId} aspectRatio={aspectRatio} autoStart={renderMode === "gameplay"} loopVideoUrl={gameplayVideoSrc || null} loopStartOffsetSeconds={Math.max(0, Number(gameplayStartOffset) || 0)} onProjectSaved={(saved) => setProject(saved)} /> : null}
